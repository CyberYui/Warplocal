...
pub(crate) struct ServerTask {
    pub(crate) source: api::Task,
    pub(crate) subagent_params: Option<SubagentParams>,
}
...
impl Task {
    ...
    pub(crate) fn into_server_created_task(
        mut self,
        task: api::Task,
    ...
    pub fn push_exchange(&mut self, exchange: AIAgentExchange) -> AIAgentExchangeId {
        let id = exchange.id;
    ...
    pub(super) fn add_messages(
        ...
        log::info!("Task::add_messages: task_id={} exchange_id={} msg_count={}", self.id, exchange_id, messages.len());
        for msg in &messages {
            match &msg.message {
                Some(api::message::Message::AgentOutput(ao)) => {
                    log::info!("Task::add_messages: AgentOutput text={}", ao.text);
                }
                Some(api::message::Message::UserQuery(uq)) => {
                    log::info!("Task::add_messages: UserQuery query={}", uq.query);
                }
                _ => {
                    log::info!("Task::add_messages: other message type");
                }
            }
        }
    ...
    fn update_exchange_from_messages(
        ...
        let output_messages: Result<Vec<AIAgentOutputMessage>, MessageToAIAgentOutputMessageError> =
            messages
                .into_iter()
                .filter_map(|m| {
                    log::info!("update_exchange_from_messages: converting message id={} type={:?}", m.id, std::mem::discriminant(&m.message));
