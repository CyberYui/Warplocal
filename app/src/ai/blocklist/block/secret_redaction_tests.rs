...
fn test_detect_secrets_multiple_secrets() {
    ...
    let text = "ABCD ghp_DUMMY_TOKEN_FOR_TESTING_PURPOSES_12345678901234 foo baz warp-server-staging.firebaseapp.com bar \n foo sk_live_DUMMY_KEY_FOR_TESTING_PURPOSES_12345678 qux foo";
    let detected_secrets = find_secrets_in_text(text);
    assert_eq!(
        detected_secrets,
        vec![
...
fn test_detect_secrets_default_regex_case_sensitivity() {
    ...
    let text = "API keys: sk_test_lowercase_test_key_for_testing SK_TEST_UPPERCASE_TEST_KEY_FOR_TESTING";
    let detected_secrets = find_secrets_in_text(text);
    assert_eq!(
        detected_secrets,
        vec![SecretRange {
