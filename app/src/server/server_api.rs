...
impl ServerApi {
    ...
    {
        log::info!("generate_multi_agent_output: CALLED, local_inference={}, task_context_tasks={}, model_base={}",
            self::ai::is_local_inference_enabled(),
            request.task_context.as_ref().map(|tc| tc.tasks.len()).unwrap_or(0),
            request.settings.as_ref().and_then(|s| s.model_config.as_ref()).map(|mc| mc.base.as_str()).unwrap_or("none")
        );
        if self::ai::is_local_inference_enabled() {
            return self.generate_multi_agent_output_local(request).await;
        }
    ...
    /// Local inference fallback for Agent Mode.
    /// Extracts the user's message from the protobuf request, calls the local LLM,
    ...
    async fn generate_multi_agent_output_local(
        ...
        let user_text = request
            ...
            .and_then(|input_type| {
                if let api::request::input::Type::UserInputs(inputs) = input_type {
                    inputs.inputs.iter().find_map(|ui| {
                        if let Some(api::request::input::user_inputs::user_input::Input::UserQuery(q)) = &ui.input {
                            Some(q.query.clone())
                        } else {
                            None
                        }
                    })
                } else if let api::request::input::Type::UserQuery(q) = input_type {
                    Some(q.query.clone())
                } else {
                    None
                }
        ...
        // Determine which API endpoint to use for local inference:
        // 1. Check if the request's model_config.base matches a custom model provider's config_key
        //    (from the request's settings.custom_model_providers, populated by ApiKeyManager)
        // 2. Fall back to ~/.warplocal/config.toml endpoint overrides
        // 3. Fall back to default Ollama config
        let model_config_key = request
            .settings
            .as_ref()
        ...
        log::info!("generate_multi_agent_output_local: ENTERED");

        // Get task ID from request (use real task ID from conversation)
        let task_id = request
            .task_context
            .as_ref()
        ...
        log::info!("generate_multi_agent_output_local: task_id={} model_config_key={:?}", task_id, model_config_key);

        // Try to find a matching custom model provider from the request's settings.
        // custom_model_providers is populated by RequestParams::new() from ApiKeyManager.
        // We need both the provider (url + key) AND the model slug (real model name).
        let custom_provider = model_config_key.and_then(|key| {
            request
                .settings
                .as_ref()
                .and_then(|s| s.custom_model_providers.as_ref())
                .and_then(|cmp| {
                    for p in &cmp.providers {
                        for m in &p.models {
                            if m.config_key == key {
                                return Some((p.base_url.clone(), p.api_key.clone(), m.slug.clone()));
                            }
                        }
                    }
                    None
        ...
        let llm_result = if let Some((provider_url, provider_key, model_name)) = custom_provider {
        ...
        let (tx, rx) = async_channel::unbounded::<Result<warp_multi_agent_api::ResponseEvent, Arc<AIApiError>>>();

        match llm_result {
            Ok(assistant_text) => {
                // Build the response stream: Init -> ClientActions(AddMessagesToTask) -> Finished
                ...
                let _ = tx.send(Ok(client_actions_event)).await;
                let _ = tx.send(Ok(finished_event)).await;
            }
            Err(e) => {
                let _ = tx.send(Err(Arc::new(AIApiError::Other(e)))).await;
            }
        }
    ...
    async fn generate_multi_agent_output_local(
        ...
        Err(Arc::new(AIApiError::Other(anyhow::anyhow!(
            "Agent mode local inference not supported on WASM"
        ))))
    }
