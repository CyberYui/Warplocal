...
// ── Local inference helpers (bypass Warp cloud, call user's own API) ──

/// Config file path for local AI inference settings.
const LOCAL_CONFIG_PATH: &str = ".warplocal/config.toml";
/// Environment variable names (fallback if config file not found).
const ENV_LOCAL_API_URL: &str = "WARP_LOCAL_API_URL";
const ENV_LOCAL_API_KEY: &str = "WARP_LOCAL_API_KEY";
const ENV_LOCAL_MODEL: &str = "WARP_LOCAL_MODEL";
...
struct LocalAiConfig {
    ...
    model: Option<String>,
    /// Optional: per-endpoint overrides keyed by model name.
    /// When a model name matches a key here, that endpoint's url/key are used.
    #[serde(default)]
    endpoints: std::collections::HashMap<String, LocalEndpointOverride>,
}

#[derive(serde::Deserialize, Default, Clone)]
struct LocalEndpointOverride {
    api_url: Option<String>,
    api_key: Option<String>,
}

/// Load AI config from `~/.warplocal/config.toml`, falling back to environment variables.
fn load_local_ai_config() -> LocalAiConfig {
    let home = std::env::var("HOME").unwrap_or_default();
    let config_path = std::path::Path::new(&home).join(LOCAL_CONFIG_PATH);
    match std::fs::read_to_string(&config_path) {
        Ok(content) => {
            match toml::from_str::<LocalAiConfig>(&content) {
                Ok(cfg) => cfg,
                Err(e) => {
        ...
        Err(_) => LocalAiConfig::default(),
    }
}

/// Local inference is always enabled by default.
/// The `call_local_llm` function will still use config/env vars for the actual API call,
/// but all server-call bypasses unconditionally return local defaults.
pub(crate) fn is_local_inference_enabled() -> bool {
    true
}

/// Calls the user's configured API endpoint with a chat completion request.
/// Reads config from `~/.warplocal/config.toml` or env vars.
/// Returns the assistant's reply text.
pub(crate) async fn call_local_llm(system_prompt: &str, user_prompt: &str) -> anyhow::Result<String> {
    call_local_llm_with_model(system_prompt, user_prompt, None).await
}

/// Call local LLM with an optional model name. When model is provided and matches
/// a custom endpoint override in config, that endpoint's URL/key are used.
pub(crate) async fn call_local_llm_with_model(
    system_prompt: &str,
    user_prompt: &str,
    model_name: Option<&str>,
) -> anyhow::Result<String> {
    let cfg = load_local_ai_config();

    // Determine which endpoint to use:
    // 1. If model_name matches a custom endpoint override, use that
    // 2. Otherwise use the default api_url/api_key from config
    let (api_url, api_key, model) = if let Some(mname) = model_name {
        if let Some(override_ep) = cfg.endpoints.get(mname) {
            let url = override_ep.api_url.clone()
                .or_else(|| cfg.api_url.clone())
                .or_else(|| std::env::var(ENV_LOCAL_API_URL).ok())
                .ok_or_else(|| anyhow::anyhow!("No API URL for model '{}'. Add to ~/.warplocal/config.toml", mname))?;
            let key = override_ep.api_key.clone()
                .or_else(|| cfg.api_key.clone())
                .or_else(|| std::env::var(ENV_LOCAL_API_KEY).ok())
                .unwrap_or_else(|| "ollama".to_string());
            (url, key, mname.to_string())
        } else {
            let url = cfg.api_url
                .or_else(|| std::env::var(ENV_LOCAL_API_URL).ok())
                .ok_or_else(|| anyhow::anyhow!("WARP_LOCAL_API_URL not set. Create ~/.warplocal/config.toml"))?;
            let key = cfg.api_key
                .or_else(|| std::env::var(ENV_LOCAL_API_KEY).ok())
                .unwrap_or_else(|| "ollama".to_string());
            let model = cfg.model
        ...
        let model = cfg.model
            .or_else(|| std::env::var(ENV_LOCAL_MODEL).ok())
            .ok_or_else(|| anyhow::anyhow!("WARP_LOCAL_MODEL not set. Create ~/.warplocal/config.toml"))?;
        (url, key, model)
    };

    let key_preview = if api_key.len() > 8 { &api_key[..8] } else { &api_key };
    log::info!("call_local_llm_with_model: url={} key={} model={}", api_url, key_preview, model);
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| anyhow::anyhow!("Local inference request failed: {}", e))?;

    if !resp.status().is_success() {
        let status = resp.status();
        let text = resp.text().await.unwrap_or_default();
        return Err(anyhow::anyhow!(
            "Local inference API returned {}: {}",
            status,
            text
        ));
    }
...
/// Call a specific LLM endpoint directly with explicit URL, key, and model.
/// Used by Agent Mode to route to user-configured custom endpoints.
pub(crate) async fn call_llm_direct(
    system_prompt: &str,
    user_prompt: &str,
    api_url: &str,
    api_key: &str,
    ...
    log::info!("call_llm_direct: POST {} model={}", full_url, model);
    ...
    if !resp.status().is_success() {
        let status = resp.status();
        let text = resp.text().await.unwrap_or_default();
        log::error!("call_llm_direct: API error {}: {}", status, text);
        return Err(anyhow::anyhow!(
            "LLM API returned {}: {}",
            status,
            text
...
impl AIClient for ServerApi {
    async fn generate_commands_from_natural_language(
        ...
        if is_local_inference_enabled() {
            let system = concat!(
                "You are a shell command generator. Given a user's natural language description, ",
                "respond with ONLY the single most appropriate shell command. No explanation, no markdown, ",
                "no formatting. Just the raw command.");
            match call_local_llm(system, &prompt).await {
                Ok(text) => {
                    let cmd = text.trim().trim_start_matches("```").trim_end_matches("```").trim();
                ...
                Err(e) => {
                    log::error!("Local inference for command generation failed: {e}");
                    return Err(GenerateCommandsFromNaturalLanguageError::Other);
                }
            }
        }
    ...
    async fn generate_dialogue_answer(
        ...
        if is_local_inference_enabled() {
            ...
            match call_local_llm(system, &user_msg).await {
                Ok(answer) => {
                ...
                Err(e) => {
                    log::error!("Local inference for dialogue failed: {e}");
                    return Err(anyhow::anyhow!("Local inference failed: {e}"));
                }
            }
        }
    ...
    async fn generate_metadata_for_command(
        ...
        // Local mode: return empty metadata
        if is_local_inference_enabled() {
            return Ok(GeneratedCommandMetadata {
                description: String::new(),
                title: String::new(),
                command: String::new(),
                arguments: vec![],
    ...
    #[cfg(not(feature = "agent_mode_evals"))]
    async fn get_request_limit_info(&self) -> Result<RequestUsageInfo, anyhow::Error> {
        // Local mode: return unlimited defaults
        if is_local_inference_enabled() {
    ...
    async fn get_feature_model_choices(&self) -> Result<ModelsByFeature, anyhow::Error> {
        if is_local_inference_enabled() {
            return Ok(ModelsByFeature::default());
        }
