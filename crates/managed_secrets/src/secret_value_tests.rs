...
fn test_serialize_anthropic_bedrock_access_key() {
    let secret = ManagedSecretValue::AnthropicBedrockAccessKey {
        aws_access_key_id: "AKIAIOSFODNN7DUMMYTEST".to_string(),
    ...
    assert_eq!(
        serialized,
        "{\"aws_access_key_id\":\"AKIAIOSFODNN7DUMMYTEST\",\"aws_secret_access_key\":\"wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY\",\"aws_session_token\":\"FwoGZXIvYXdzEBY\",\"aws_region\":\"us-east-1\"}"
    );
}
...
fn test_serialize_anthropic_bedrock_access_key_without_session_token() {
    ...
    assert_eq!(
        serialized,
        "{\"aws_access_key_id\":\"AKIAIOSFODNN7DUMMYTEST\",\"aws_secret_access_key\":\"wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY\",\"aws_region\":\"us-east-1\"}"
    );
    assert!(
...
fn test_debug_representation_no_secrets_anthropic_bedrock_access_key() {
    ...
    let debug_representation = format!("{:?}", secret);
    assert!(
        !debug_representation.contains("AKIAIOSFODNN7DUMMYTEST"),
